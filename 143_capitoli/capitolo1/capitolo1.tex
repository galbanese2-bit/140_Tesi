...
La giunzione tra i singoli cavi avviene nei punti di incrocio mediante boccole di compressione, anch'esse in acciaio inossidabile. Questa configurazione conferisce una rigidezza geometrica controllata e un'elevata resistenza allo scorrimento dei nodi. La tenuta delle boccole di serraggio garantisce il comportamento globale del sistema, perciò la loro resistenza rappresenta un fattore fondamentale per la rete. Difatti, nell'\textit{European Assessment Document} \cite{EAD-XTEND} che ne regola la progettazione e ne definisce le prove che garantiscono le capacità del prodotto, molta attenzione è prestata nel definire i test di resistenza di questi elementi nelle varie configurazioni previste nel relativo \textit{European Technical Assessment} \cite{ETA-XTEND}.
...
\begin{itemize}
    \item \textit{Proiezioni anticaduta e parapetti trasparenti:} utilizzato come tamponamento di passerelle, ponti, scale e balauastre, assicura la conformità alle normative sulla sicurezza senza ostacolare la visibilità e l'illuminazione naturale. 
    \item \textit{Bio-architettura e facciate verdi:} la rete funge da sottostruttura di supporto per specie vegetali rampicanti su grandi superfici verticali. Il sistema che si viene a creare, oltre alla funzione in termini di sicurezza, produce un ombreggiamento naturale che contribuisce alla mitigazione dell'assorbimento di calore derivante dall'irraggiamento solare e che contribuisce a ridurre l'effetto dell'isola di calore urbana ed al miglioramento delle prestazioni termiche dell'involucro.
    \item \textit{Strutture tridimensionali e grandi campate:} la natura flessibile della maglia consente di adattarsi a superfici a doppia curvatura (paraboloidi, forme organiche o tensostrutture tridimensionali), minimizzando l'impatto visivo delle sottostrutture di ancoraggio.
\end{itemize}
...
\begin{itemize}
    \item \textit{Trasparenza del sistema:} l'elevato rapporto tra vuoto e pieno (con permeabilità visiva che può superare il $90\%$) riduce drasticamente l'impatto visivo dell'opera, garantendo una quasi totale trasparenza e l'integrazione con il contesto paesaggistico. 
    \item \textit{Elevato rapporto resistenza/peso:} la leggerezza propria del sistema minimizza i carichi permanenti portati gravanti sulle strutture portanti e sugli elementi di ancoraggio di bordo, semplificando la progettazione delle sottostrutture. 
    \item \textit{Durabilità e manutenzione:} l'impiego di acciai inossidabili (AISI 316) assicura un'elevata resistenza alla corrosione anche in ambienti atmosferici aggressivi (es: marini o industriali), annullando la necessità di trattamenti protettivi superficiali periodici.
    \item \textit{Incombustibilità:} la natura metallica dei componenti garantisce una reazione al fuoco minima, aspetto cruciale per l'impiego su facciate ed edifici pubblici soggetti a severe normative antincendio. 
\end{itemize}

Il ruolo svolto da questo sistema nei confronti del rischio di caduta dall'alto rende necessaria la sua capacità nel resistere ad azioni ortogonali. In merito a questa capacità in \cite{EAD-XTEND} è definito un test di impatto dinamico mediante un corpo con massa e velocità di impatto predefinite. \'E importante notare come il test di impatto debba essere eseguito prendendo in considerazione quanto previsto in merito in \cite{EN1263-1}. Difatti il sistema \textit{X-TEND} svolge gli stessi compiti delle reti di sicurezza, differendo nel materiale costituente e nei nodi che caratterizzano la giunzione tra i vari cavi. 
...
Nel presente lavoro di tesi, la ricerca della configurazione di equilibrio della rete sotto peso proprio viene impostata secondo una prospettiva concettualmente opposta rispetto all'approccio FDM. Sulla base della formulazione posizionale sviluppata da Valvo \cite{Valvo1,Valvo2}, si determina infatti lo stato di equilibrio a partire da una rete di geometria e caratteristiche meccaniche note a priori.
...
L'approccio posizionale consente così di affrontare in un quadro unitario sia la fase di \textit{form-finding} inziale sia la successiva valutazione delle capacità di assorbimento dell'energia potenziale elastica della rete. L'integrazione di tale solutore con la formulazione per il problema di contatto descritta da Fisicaro \cite{Fisicaro} costituisce l'obiettivo primario del presente lavoro.
