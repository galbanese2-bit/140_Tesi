...
Dal punto di vista della meccanica, le funi e le reti di funi si distinguono nettamente dalle tipologie strutturali tradizionali dell'ingegneria civile. L'assenza di rigidezza a flessione e a compressione fa sì che esse lavorino unicamente a trazione, adattando la propria geometria per raggiungere l'equilibrio in funzione dei carichi e dei vincoli applicati. La risposta di questi sistemi è quindi dominata da una forte non linearità geometrica, dove la configurazione finale può differire in modo significativo da quella iniziale di riferimento. 

In questo capitolo vengono presentati i principali modelli teorici e numerici utilizzati per descrivere il comportamento delle funi e, successivamente, delle reti di funi. La prima parte è dedicata alla meccanica delle funi, analizzata attraverso tre approcci di complessità crescente: il modello inestensibile, il modello ad elasticità diffusa e una formulazione alle differenze finite basata sulle equazioni indefinite di equilibrio. Questi modelli permettono di affrontare i due problemi classici della meccanica delle funi — la fune soggetta al peso proprio e la fune soggetta a un carico trasversale uniformemente distribuito per unità di lunghezza — evidenziando analogie, differenze e limiti delle diverse formulazioni.

La seconda parte introduce invece la modellazione delle funi tramite metodi numerici agli elementi finiti, presentando una formulazione posizionale del Finite Element Method (FEM), nota come Position-based Finite Element Formulation (PFEF). Questo approccio semplifica la gestione dei grandi spostamenti e delle deformazioni finite, fornendo l'ossatura computazionale per lo studio di sistemi strutturali complessi.

Infine, l'ultima parte del capitolo estende il modello alla trattazione delle reti di funi, intese come strutture discrete costituite da un insieme di funi interconnesse. Vengono approfonditi gli aspetti topologici, la compilazione della matrice di connettività e le strategie per la definizione delle differenti geometrie di maglia e delle condizioni vincolo al bordo.

L'obiettivo complessivo del capitolo è fornire un percorso teorico-numerico organico e progressivo che, muovendo dallo studio analitico del singolo elemento continuo, conduca alla formulazione del modello numerico discreto rete di funi.

\section{La meccanica delle funi}
La meccanica della singola fune costituisce il punto di partenza per la comprensione del comportamento delle reti e delle strutture flessibili basate su elementi esclusivamente tesi. In questa sezione vengono analizzati i modelli teorici di riferimento che descrivono il comportamento statico delle funi, con particolare attenzione ai due problemi classici della disciplina: la fune soggetta a un carico verticale distribuito lungo l'orizzontale e la fune soggetta al peso proprio lungo la propria linea d'asse.
...
La trattazione è articolata secondo un percorso di complessità crescente basato su tre approcci:
\begin{itemize}
   \item \textbf{Il modello inestensibile:} rappresenta la formulazione geometrica classica, utile per determinare la forma assunta dalla fune in equilibrio, ma priva della descrizione del processo deformativo del materiale;
   \item \textbf{Il modello ad elasticità diffusa:} introduce la deformabilità assiale dell'elemento, consentendo di descrivere la risposta sia in regime di fune lasca che di fune fortemente tesa;
   \item \textbf{Il modello alle differenze finite:} basato sulla risoluzione diretta alle equazioni differenziali indefinite di equilibrio, permette di analizzare in modo rigoroso la distribuzione puntuale dello stato tensionale lungo il cavo. 
\end{itemize}
...
\subsubsection{L'equilibrio della fune soggetta ad un carico trasversale distribuito}
Consideriamo la configurazione finale di una fune soggetta ad un carico verticale distribuito $p$. 
...
La componente orizzontale della forza di tiro risulta quindi:
\begin{equation}
   H = \frac{pl^{2}}{8f}
   \label{eq:equazione_tiro}
\end{equation}
con $f$ freccia massima della fune.

Misurando $p$ e le grandezze geometriche $l$ ed $f$, è immediato ricavare l'intensità della componente orizzontale $H$ della forza di tiro. Di conseguenza, la formula permette di conoscere il tiro di una fune già installata, quando la freccia massima $f$ può essere misurata, ma non consente di determinare a priori il tiro necessario per ottenere una configurazione desiderata. 
...
Nel modello inestensibile, la lunghezza della fune nella configurazione deformata coincide necessariamente con la lunghezza iniziale $L_0$. Tuttavia, nella pratica ingegneristica si presentano frequentemente situazioni in cui è necessario tener conto della variazione della lunghezza in opera rispetto al suo valore iniziale. 
\'E possibile estendere a tali casi la modellazione della trave inestensibile introducendo una \textit{Lunghezza efficace} della fune, definita come: 
\begin{equation}
    L_{eff} = L_0 - \Delta L_0 = L_0(1 - \varepsilon_0) 
    \label{eq:lunghezza_efficace}
\end{equation}
dove $\Delta L_0$ rappresenta un accorciamento imposto in fase di montaggio. In questo contesto $\varepsilon_0$ non rappresenta una deformazione imposta al materiale, ma è la percentuale di lunghezza iniziale del cavo che produce l'accorciamento desiderato. L'idea è quella di considerare la fune come inestensibile, ma con una lunghezza iniziale variabile. 

\subsubsection{L'equazione di congruenza}
Nei modelli analizzati, che trattano la fune come inestensibile, imponendo l'equilibrio si ottiene una relazione tra la forma della fune e l'intensità della componente orizzontale $H$ del tiro. Tuttavia, né nel caso della parabola, né in quello della catenaria, la configurazione finale è nota a priori: essa dipende proprio dal valore di $H$.
...
\end{equation}
che combina equilibrio e congruenza e permette di determinare l'intensità della componente orizzontale $H$ del tiro. 
...
\end{itemize}
Il modello della fune inestensibile è utile per determinare la configurazione assunta dalla fune quando viene raggiunto l'equilibrio statico, ma non è in grado di rappresentare l'evoluzione deformativa o gli effetti della deformabilità assiale in quanto non tiene conto delle relazioni costitutive del materiale. 
...
Il modello analizzato in questa sezione introduce un legame costitutivo esplicito tra spostamenti, deformazioni e tensioni. Questo approccio permette di superare le limitazioni intrinseche del modello inestensibile, le quali risultano fortemente vincolanti nella pratica progettuale.

Inizialmente verrà introdotto il legame costitutivo del cavo; successivamente, verrà formulata l'equazione di congruenza che tiene conto dell'allungamento assiale, al fine di determinare il tiro orizzontale $H$ attraverso una strategia numerica.
...
\end{equation}
Coerentemente con le \cref{fig:fune_catenaria,fig:fune_carico_distr}, definiamo $s_0$ ed $s$, rispettivamente, l'ascissa curvilinea della fune nella configurazione iniziale e finale. 
Assumendo l'ipotesi di piccole deformazioni elastiche del materiale, la deformazione assiale può essere espressa mediante la relazione cinematica:
\begin{equation}
...
Combinando la relazione \eqref{eq:deformazione_assiale} con il legame costitutivo \eqref{eq:legame_costitutivo_cavo}, è possibile esplicitare il tratto differenziale iniziale $ds_0$ in funzione della configurazione deformata corrente e dello sforzo agente: 
\begin{equation}
...
Per stabilire la condizione di congruenza del problema elastico, si consideri la lunghezza totale della fune nella configurazione finale $L(H)$. Essa sarà pari alla lunghezza iniziale $L_0$ maggiorata dell'allungamento totale elastico $\Delta L$ indotto dai carichi:
\begin{equation}
   L(H) - \Delta L - L_0 = 0
...
\end{equation}
Ricordando che lo sforzo normale lungo il profilo della fune è legato alla componente orizzontale $H$ del tiro, dalla relazione:
\begin{equation}
...
Consideriamo anche in questo caso la necessità, dato un cavo di lunghezza inizale $L_0$, di sottoporlo ad un accorciamento $\Delta L_0$. Tale condizione modella fenomeni fisici reali importanti nell'ingegneria, come le operazioni di pre-tensionamento meccanico agli ancoraggi, i difetti di montaggio in cantiere e le contrazioni termiche indotte. Coerentemente con quanto definito per il modello inestensibile, si introduce la \textit{lunghezza efficace} della fune in configurazione indeformata, definita come:
\begin{equation}
    L_{eff} = L_0 - \Delta L_0 = L_0(1 - \varepsilon_0)
    \label{eq:lunghezza_efficace_elastica}
\end{equation}
dove $\varepsilon_0$ rappresenta l'aliquota di accorciamento percentuale impresso rispetto alla geometria iniziale. L'introduzione di $L_{eff}$ modifica l'equazione di congruenza. La fune vincolata agli appoggi posti a distanza $l$ tra loro, reagisce alla contrazione sviluppando internamente uno stato tensionale di trazione, nel caso in cui $L_{eff}<l$, mentre produce una variazione geometrica pari a quella prodotta nel caso inestensibile, quando $L_{eff}>l$. Al fine di esplicitare il ruolo di ciascuna componente deformativa la lunghezza totale della fune nella configurazione deformata $L$ può essere vista come la somma algebrica della lunghezza efficace $L_{eff}$, dell'accorciamento di montaggio $\Delta L_0$ e dell'allungamento elastico $\Delta L$:
\begin{equation}
...
\vspace{1em}
Prendiamo ad esempio una fune in acciaio (possiamo assumere che il modulo elastico sia pari a $E = 210\,\text{GPa}$ e che la densità valga $7850\,\text{kg/m}$), di area pari ad $A = \pi r^{2}$ con $r = 1\,\text{cm}$, soggetta all'azione del solo peso proprio lineare ($\gamma = \rho g A$) considerata agente sulla lunghezza della fune stessa, la cui distanza tra gli appoggi è pari a $l$. Ipotizzando di conoscere la lunghezza iniziale $L_0$ e variando la $L_{eff}$ per mezzo di un accorciamento $\Delta L$, grazie alla \cref{fig:estensibilevsinestensibile1} possiamo vedere come attraverso il modello estensibile si possano considerare anche problemi la cui $L_{eff}$ sia minore di $l$.
...
In \cref{fig:estensibilevsinestensibile3,fig:estensibilevsinestensibile4} emergono chiaramente i limiti del modello inestensibile. 
...
\end{equation}
Imponendo il parallelismo tra l'azione interna dello sforzo normale e la tangente locale alla linea d'asse della configurazione corrente, si ricavano le componenti dello sforzo normale in direzione orizzontale e verticale:
\begin{equation}
...
\vspace{1em}
Analizziamo quindi l'equilibrio di un tratto infinitesimo di fune. Consideriamo un segmento nella configurazione deformata, l'equilibrio alla traslazione in direzione orizzontale $x$ si impone attraverso la relazione:
\begin{equation}
   N_x(s_2) - N_x(s_1) = 0
\end{equation}
...
\begin{itemize}
   ...
   \item  $\underline{R}(\underline{u})$, è la funzione dei residui, i cui dominio e codominio sono tali per cui: $R : \mathbb{R}^{2(n-1)} \rightarrow \mathbb{R}^{2n-1}$;
...
\end{equation}
Dove $k$ rappresenta l'indice dell'iterazione corrente e $[J(\underline{u}^{(k)})]$ è la matrice Jacobiana della funzione dei residui. A differenza del caso continuo, in cui il problema dipende dall'unica variabile $s_0$, in questo contesto discreto le variabili da controllare sono pari a $2n - 1$. Di conseguenza, le componenti della Jacobiana sono definite come: 
\begin{equation}
...
Viene proposto un modello agli elementi finiti (FE) di reti di funi con masse nodali concentrate e elementi cavo del primo ordine. In linea con la formulazione posizionale degli elementi finiti (\textit{Position-based Finite Element Formulation} - PFEF) proposta da P.S.Valvo \cite{Valvo1,Valvo2}, assumiamo come incognite principali del problema le posizioni nodali, anzichè gli spostamenti nodali, che sono comunemente adottati nelle analisi agli elementi finiti. Come risultato, la matrice di rigidezza elastica secante del sistema risulterà essere simmetrica e non sarà necessaria l'adozione di due distinti sistemi di riferimento (locale e globale), in quanto ogni grandezza è riferita a un unico sistema di coordinate globali. 

I grandi spostamenti e le deformazioni finite sono descritte per mezzo del tensore di deformazione di Green-Lagrange. si assume che gli elementi cavo possano reagire solamente in trazione (\textit{tension-only}) mediante una relazione lineare tra la deformazione assiale e la corrispondente componente del secondo tensore delle tensioni di Piola-Kirchhoff. Questa legge costitutiva è nota in letteratura come modello di materiale di Saint-Venant-Kirchhoff. 
...
Assumendo funzioni di forma lineari per gli elementi, in Valvo \cite{Valvo1,Valvo2} la matrice di rigidezza elastica secante del singolo cavo $S_e \in \mathbb{R}^{6\times6}$ viene espressa mediante la relazione:
\begin{equation}
...
\end{equation}
dove $\overline{a}_e$ è l'area della sezione dell'elemento, valutata in configurazione di riferimento. Allo stesso tempo, la matrice di rigidezza elastica tangente $T_e \in \mathbb{R}^{6\times6}$ si ottiene differenziando le forze interne e può essere espressa come:
\begin{equation}
...
\end{equation}  
che, secondo quanto dimostrato in Fisicaro \cite{Fisicaro} può essere riscritta in forma compatta come: 
\begin{equation}
...
\end{equation}  
Dalle equazioni \eqref{eq:matrice_secante} e \eqref{eq:matrice_tangente2} la dipendenza esplicita delle matrici dal vettore delle posizioni nodali dell'elemento $\underline{x}_e$ è racchiusa implicitamente nella lunghezza nominale $l_e$. 

\vspace{1em}
Come detto in precedenza, il modello a massa concentrata adottato prevede di ripartire la massa totale dell'elemento sui nodi di estremità. In Clough e Penzien \cite{Penzien}, la matrice di massa globale del singolo elemento $M_e \in \mathbb{R}^{6\times6}$ viene espressa come:
\begin{equation}
   M_e = 
   ...
   \end{bmatrix}
\end{equation}
dove $\overline{\rho}_e$ è la densità di massa uniforme nella configurazione di riferimento, e $0 \in \mathbb{R}^{3\times3}$ è la sottomatrice nulla. La struttura diagonale a blocchi riflette il fatto che ciascun nodo riceve metà della massa totale dell'elemento, senza accoppiamenti inerziali reciproci. 
...
\begin{equation}
   \underline{f}_{g,e} = \frac{1}{2}\overline{\rho}_e \overline{a}_e \overline{l}_e a_g
\left\{
\begin{array}{c}
0 \\
0 \\
...
\right\}
\end{equation}
dove $a_g \approx 9.81 \, \text{m/s}^2$ è il modulo dell'accelerazione di gravità. Il vettore rappresenta la forza peso applicata ai due nodi dell'elemento, orientata lungo la direzione verticale del sistema di riferimento (assumendo l'asse $x_3$ rivolto verso il basso).  

\vspace{1em}
Al fine di formalizzare l'assemblaggio delle matrici appena introdotte e relative all'intero sistema meccanico introduciamo la matrice di localizzazione dell'$i$-esimo nodo $L_i \in \mathbb{R}^{3\times3n}$ e la matrice di assemblaggio dell'$e-simo$ elemento $A_e \in \mathbb{R}^{6\times3n}$. 
La matrice di localizzazione è definita come una matrice nulla, eccetto per le componenti corrispondenti alle colonne da $3i -2$ a $3i$ dove è posizionata la matrice identità $I \in \mathbb{R}^{3\times3}$ .
\begin{equation}   
...
\begin{equation}
 \underline{f}_{g} = \sum_{e=1}^{m} A_e^T \underline{f}_{g,e}
...
\end{equation}
Infine, viene integrato nel modello uno smorzamento viscoso proporzionale secondo la formulazione di Rayleigh. In Clough e Penzien \cite{Penzien}, la matrice di smorzamento $D \in \mathbb{R}^{3n \times 3n}$ è ottenuta come combinazione lineare della matrice di massa e della matrice di rigidezza elastica tangente, quest'ultima valutata nella configurazione di riferimento (o indeformata) $\underline{\overline{x}}$: 
\begin{equation}
...
Le equazioni differenziali che governano il problema dell'equilibrio dinamico non lineare possono essere dedotte secondo approcci differenti. In questa trattazione, come in Fisicaro\cite{Fisicaro}, si segue il metodo Lagrangiano, successivamente anche alla trattazione del contatto tra la rete e il corpo impattante.
...
L'energia potenziale gravitazionale del sistema meccanico può essere espressa rispetto alla configurazione di riferimento come:
\begin{equation}
   \mathcal{V} = \mathcal{U} - \mathcal{W}
   \label{eq:energia_potenziale_totale}
\end{equation}
Impiegando un approccio a masse concentrate, la massa degli elementi in cui il sistema è discretizzato viene concentrata nei nodi del sistema in funzione del peso proprio degli elementi in configurazione indeformata. Sia $m_i$ la massa concentrata nell'$i$-esimo nodo; la forza peso relativa a tale massa è pari a $\underline{f}_{gi} = m_i\underline{a}_g$, dove $\underline{a}_g \in \mathbb{R}^3$ indica il vettore accelerazione di gravità. Orientando il sistema di riferimento cartesiano globale con l'asse $x_3$ diretto verso il basso, il vettore $\underline{a}_g$ assume la forma:
\begin{equation}
   \underline{a}_g = a_g
   \left\{
   \begin{array}{c}
   ...
   \right\}
\end{equation}
Perciò il lavoro compiuto della forza peso quando l'$i$-esimo nodo subisce uno spostamento $\underline{u}_i = \underline{x}_i - \overline{\underline{x}}_i$ è pari al prodotto scalare:
\begin{equation}
   \mathcal{W}_i = \underline{f}_{gi}^{T} \underline{u}_i
\end{equation}
Il lavoro totale svolto dalla forza peso sul sistema meccanico può essere scritto come la sommatoria dei contributi energetici relativi agli $n$ nodi del sistema secondo la relazione: 
\begin{equation}
...
\end{equation}
Seguendo la formulazione proposta in Valvo \cite{Valvo1,Valvo2}, l'energia potenziale elastica dell'$e$-esimo elemento si esprime come:
\begin{equation}
   \mathcal{U}_e = \int_{\overline{\Omega}_e} \overline{\varphi}\, dx_1 dx_2 dx_3
\end{equation}
...
\end{equation}
L'energia potenziale elastica totale dell'intero sistema meccanico è espressa dalla somma dei contributi sui singoli $m$ elementi: 
\begin{equation}
   \mathcal{U} = \frac{1}{2} \sum_{e=1}^{m} \sigma_{11} e_{11} \overline{a}_e \overline{l}_e
   \label{eq:energia_potenziale_elastica}
\end{equation}
Per calcolare il gradiente dell'energia potenziale elastica della rete rispetto al vettore delle posizioni nodali $\underline{x}$, valutiamo in primis la derivata dell'energia potenziale elastica dell'$e$-esimo elemento $\mathcal{U}_e$. Ricordando le espressioni \eqref{eq:deformazione_green_lagrange} e \eqref{eq:tension_only2}, si ha:
\begin{equation}
   \frac{\partial \mathcal{U}_e}{\partial \underline{x}} = \frac{1}{2} \overline{a}_e \overline{l}_e \frac{\partial (\sigma_{11} e_{11})}{\partial {\underline{x}}} = \frac{1}{2} \sigma_{11} \frac{\overline{a}_e}{\overline{l}_e} \frac{\partial l_e^2}{\partial {\underline{x}}} = \frac{1}{2} \sigma_{11} \frac{\overline{a}_e}{\overline{l}_e} \frac{\partial l_e^2}{\partial {\underline{x}}}
\end{equation}
Sommando i contributi relativi a tutti gli $m$ elementi del sistema, si ottiene la derivata dell'energia potenziale elastica totale:
\begin{equation}
   \frac{\partial \mathcal{U}}{\partial {\underline{x}}} = \frac{1}{2} \sum_{e=1}^{m} \overline{a}_e \overline{l}_e \frac{\partial {\sigma_{11} e_{11}}}{\partial {\underline{x}}} = \frac{1}{2} \sum_{e=1}^{m} \sigma_{11} \frac{\overline{a}_e}{\overline{l}_e} \frac{\partial l_e^2}{\partial {\underline{x}}}
...
Per tener conto delle forze dissipative, estendiamo il formalismo lagrangiano introducendo la funzione di dissipazione di Rayleigh definita in Clough e Penzien \cite{Penzien}:
\begin{equation}
...
La formulazione Lagrangiana dell'equilibrio di un sistema meccanico espresso in soli gradi di libertà (\textit{gdl}) liberi non esplicita le forze reattive generate dai vincoli cinematici. In Amendola \cite{Amendola}, l'ipotesi di vincoli cinematici ideali implica che il lavoro virtuale svolto dalle reazioni vincolari $\underline{r}_{\text{vinc}}$ sia nullo per qualsiasi spostamento virtuale ammissibile. Per esplicitare il contributo di tali reazioni, risulta pertanto necessario riformulare il problema introducendo opportunamente i vincoli e le relative incognite reattive.
...
\begin{itemize}
   ...
   \item $f$: il numero di \textit{gdl} liberi (\textit{free}) del sistema meccanico;
...
Per incorporare tali vincoli nel bilancio energetico senza ridurre a priori le dimensioni del sistema, come in Fisicaro \cite{Fisicaro}, è possibile definire la funzione Lagrangiana $\mathcal{L}$ del sistema come segue:
\begin{equation}
   \mathcal{L} \left(\underline{q}, \dot{\underline{q}}\right) = \mathcal{K} - \mathcal{V} + \underline{\lambda}^T \underline{g}(\underline{x})
...
Il vettore delle coordinate generalizzato $\underline{q}$ è definito come:
\begin{equation}
   \underline{q} =
   \left\{
   \begin{array}{c}
...
Sostituendo l'espressione \eqref{eq:funzione_lagrangiana_vincolata} in \eqref{eq:lagrangiana_q}, si ottiene così il sistema algebrico-differenziale accoppiato di $3n + c$ equazioni in $3n+c$ incognite che governa la dinamica vincolata della rete:
\begin{equation}
   \begin{cases}
...
I risultati ottenuti nel presente lavoro mediante la formulazione proposta in Fisicaro \cite{Fisicaro} trovano riscontro, per questo caso degenere, con quanto riportato in Morin \cite{Morin2008}.

Si evidenzia come, in analogia con la trattazione sviluppata in Fisicaro \cite{Fisicaro} relativamente ai problemi dinamici vincolati, l'impiego dei Moltiplicatori di Lagrange in presenza di vincoli strettamente bilaterali non richieda l'introduzione di variabili ausiliarie, necessarie invece in presenza di vincoli di disuguaglianza o monolaterali. La natura bilaterale del vincolo consente infatti di esprimere le condizioni cinematiche al contorno direttamente sotto forma di uguaglianze $\underline{g}(\underline{x}) = \underline{0}$.
...
Pertanto, nel prosieguo del lavoro viene adottato un approccio più affine a quello della metodologia degli elementi finiti, come proposto anche in Felippa \cite{felippa2004ifem}. Il metodo consiste nel risolvere il problema meccanico, in questo caso statico o quasi-statico, considerando dapprima la struttura libera nello spazio euclideo. 
\begin{equation}
...
Si evidenzia inoltre che, per effetto della struttura a blocchi della Jacobiana trasposta $J^T = \begin{bmatrix} 0_{f \times c} & I_{c \times c} \end{bmatrix}$, il termine $J^T \underline{\lambda}$ non fornisce alcun contributo in corrispondenza delle prime $f$ equazioni. Le reazioni vincolari risultano così algebricamente disaccoppiate dalla cinematica dei gradi di libertà liberi, consentendo la determinazione del vettore $\underline{x}_f$ in modo del tutto indipendente dal vettore dei moltiplicatori $\underline{\lambda}$.
...
\begin{enumerate}
   ...
   \begin{equation}
      ...
      \label{eq:vettore_x_partizionato}
   \end{equation}
...
\begin{equation}
   K(\underline{x}^{(k)}) \Delta\underline{x}^{(k)} = - \underline{R}(\underline{x}^{(k)})
   %   \underline{x}^{(k+1)} = \underline{x}^{(k)} - [J_R(\underline{x}^{(k)})]^{-1} R(\underline{x}^{(k)})
\end{equation}
dove $\Delta \underline{x}^{(k)}$ rappresenta l'incremento del vettore delle posizioni nodali all'iterazione $k$-esima, mentre $K(\underline{x}^{(k)}) \in \mathbb{R}^{2n \times 2n}$ è la matrice ottenuta applicando lo Jacobiano al residuo statico $\underline{R}(\underline{x}^{(k)})$. 
...
\end{equation}
Questo approccio analitico, basato sulla scomposizione della rigidezza tangente come somma del contributo secante attuale $S_e$ e del contributo dovuto alla variazione geometrica $\frac{\partial S_e}{\partial \underline{x}_e} \underline{x}_e$, si inserisce nel solco della letteratura classica ed è riportato in Fisicaro \cite{Fisicaro}. In tale ambito, l'introduzione di $T_e$ nella soluzione mediante il metodo di Newton-Raphson per problemi di elasticità non lineare formulati in coordinate nodali è descritta in Wriggers \cite{Wriggers}.

\vspace{1em}
Nel caso specifico dell'elemento cavo qui trattato, introducendo il legame cinematico di Green-Lagrange \eqref{eq:deformazione_green_lagrange} e la legge costitutiva \textit{tension-only} \eqref{eq:tension_only2} all'interno del termine differenziale $\frac{\partial S_e}{\partial \underline{x}_e} \underline{x}_e$, l'esplicitazione di tale derivata conduce direttamente, come dimostrato in Fisicaro \cite{Fisicaro} nell'appendice A, alla formulazione compatta della matrice di rigidezza tangente elementare presentata nell'equazione \eqref{eq:matrice_tangente2}. In essa, la dipendenza dallo stato cinematico corrente rimane condensata implicitamente nel solo parametro della lunghezza nominale $l_e$.

Poichè l'operatore globale delle forze interne è ottenuto per assemblaggio lineare dei contributi dei singoli elementi tramite la relazione \eqref{eq:matrice_rigidezza}, la differenziazione del residuo statico definisce la matrice di rigidezza tangente della rete $T(\underline{x})$. Ne consegue che la matrice del sistema statico $K(\underline{x})$ coincide con la matrice di rigidezza tangente della rete, e le sue componenti possono essere espresse semplicemente come:
\begin{equation}
   K_{ij}(\underline{x}) = \frac{\partial (S_i(\underline{x})\underline{x})}{\partial x_j} = T_{ij}(\underline{x}),
   \qquad i,j = 1,\dots,2n.
...
A titolo esemplificativo, consideriamo le mesh pattern standard $Q$ e $D$ previste dalla norma EN 1263-1 \cite{EN1263-1} e riportate in \cref{fig:pattern_mesh}. Per entrambe le configurazioni, l'ordinamento lessicografico definisce la sequenza con cui avviene la numerazione dei nodi della maglia, sebbene venga declinato secondo logiche di scansione differenti: 
\begin{itemize}
